...
- A host (the device running the game) can broadcast their score sheet over local network / Bluetooth to nearby phones running Farkle. No internet, no accounts.
...
- Discovery: auto-discover nearby hosts by default; a 4-digit room code is shown on the host as a backup for tricky networks. Joiner can pick a discovered host or enter the code.
